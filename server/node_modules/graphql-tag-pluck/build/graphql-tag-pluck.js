...
var supportedExtensions = gqlExtensions.concat(jsExtensions);
...
/* harmony default export */ __webpack_exports__["default"] = (function (code, out) {
  ...
  var _defaults$options = _objectSpread({}, defaults, options),
      defaultGqlIdentifierName = _defaults$options.defaultGqlIdentifierName,
      gqlMagicComment = _defaults$options.gqlMagicComment,
      gqlPackName = _defaults$options.gqlPackName; // Prevent case related potential errors


  defaultGqlIdentifierName = defaultGqlIdentifierName.toLowerCase();
  gqlMagicComment = gqlMagicComment.toLowerCase();
  gqlPackName = gqlPackName.toLowerCase(); // Will accumulate all template literals

  var gqlTemplateLiterals = []; // By default, we will look for `gql` calls

  var gqlIdentifierName = defaultGqlIdentifierName;
  ...
  return {
    CallExpression: {
      enter: function enter(path) {
        ...
        if (path.node.callee.name == 'require' && path.node.arguments[0].value == gqlPackName) {
        ...
        if (_babel_types__WEBPACK_IMPORTED_MODULE_0__["isIdentifier"](path.node.callee) && path.node.callee.name == gqlIdentifierName && _babel_types__WEBPACK_IMPORTED_MODULE_0__["isTemplateLiteral"](arg0)) {
    ...
    ImportDeclaration: {
      enter: function enter(path) {
        ...
        var gqlImportSpecifier = path.node.specifiers.find(function (importSpecifier) {
          return _babel_types__WEBPACK_IMPORTED_MODULE_0__["isImportDefaultSpecifier"](importSpecifier);
        });
    ...
    TaggedTemplateExpression: {
      exit: function exit(path) {
        ...
        // e.g. gql `query myQuery {}` -> query myQuery {}
        if (!_babel_types__WEBPACK_IMPORTED_MODULE_0__["isIdentifier"](path.node.tag) || path.node.tag.name != gqlIdentifierName) {
          return;
        }
